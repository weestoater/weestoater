...
function makePalette(colorScheme, saltColorOverrides) {
  const colorNames = Array.from(
    Object.keys(saltColorOverrides != null ? saltColorOverrides : colorMap.saltColorMap)
  );
  const colorArray = [];
  for (const colorName of colorNames) {
    if (colorName.includes(colorScheme) && !(colorName === colorScheme)) {
      colorArray.push(
        saltColorOverrides ? saltColorOverrides[colorName] : colorMap.saltColorMap[colorName]
      );
    }
  }
  return colorArray;
