...
function getColorNameByHexValue(hexValue, disableAlpha = false, saltColorOverrides, disableFallBackToHex = false) {
  ...
  let colorName = Object.keys(saltColors).find((key) => {
    if (saltColors[key]) {
      ...
      return Color.makeColorFromRGB(Number(r), Number(g), Number(b)).hex === (hexNoAlpha == null ? void 0 : hexNoAlpha.toLowerCase());
    }
    return 0;
  });
  if (colorName) {
...
export { convertColorMapValueToHex, getColorNameByHexValue, getHexValue, hexValueWithoutAlpha, isValidHex };
