...
import { countChildItems, isDisabled, isFocusable, isHeader, isGroupNode, sourceItems, childItems, isParentPath, replaceCollectionItem } from './utils/collection-item-utils.js';
...
const useCollectionItems = ({
  ...
  const filterPattern = useRef((_a = options.filterPattern) != null ? _a : "");
  const {
  ...
  const stringToCollectionItem = useCallback(
    (value) => {
      const toCollectionItem2 = (item) => {
        const collectionItem = flattenedDataRef.current.find(
          (i) => isValidElement(i.value) ? i.label === item : i.value !== null && itemToString$1(i.value) === item
        );
        if (collectionItem) {
          return collectionItem;
        }
