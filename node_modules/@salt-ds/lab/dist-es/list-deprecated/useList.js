import { useId, useIsFocusVisible, useControlled, ownerDocument, useForkRef } from '@salt-ds/core';
...
function useList(props = {}) {
  ...
  const handleRangeSelect = useCallback(
    (event, index) => {
      ...
      (_a = ownerDocument(event.currentTarget).getSelection()) == null ? void 0 : _a.removeAllRanges();
      setSelectedItem(
        nextItems
      );
      if (onChange) {
  ...
  const handleMouseLeave = (event) => {
    if (focusVisible && id) {
      const rootNode = (rootRef.current || ownerDocument(event.currentTarget)).getRootNode();
